...
namespace wf14_winforms_thread
{
    public partial class FrmMain : Form
    {
        ...
        {
            Worker.WorkerSupportsCancellation = true;   // 비동기 취소 가능
            Worker.WorkerReportsProgress = true;    // 진행률 업데이트 보고 가능
        }

        // 백그라운드로 일 진행 // Thread.Start()
        private void Worker_DoWork(object sender, DoWorkEventArgs e)
        {
            BackgroundWorker worker = sender as BackgroundWorker;   // 인자값으로 BackgroundWorke 생성 // 얕은 복사 - Worker 주소 참조
        ...
        {
            // arg는 0~91사이 91보다 크면 long 오버플로우 발생
            if(arg < 0 || arg > 91)
            {
            ...
            else
            {
                ...
                {
                    // arg만 계산에 필요한 매개변수, worker, e 중간에 취소할 때 필요한 Backworger 복사본
